...
NUM_PER_IMG = 4
OVERLAP = 20
DEBUG_DRAW = False # 生成数据集时建议设为 False
...
for i in range(NUM_SAMPLES):
    ...
    yolo_labels = []
    yolo_lines = []  # ❗ 替换点：改为直接存储拼接好的 17 值字符串
    x_pos = 100
    ...
    for data in processed_imgs_data:
        ...
        # 计算并约束 BBox 边界（防止越界报错）
        xmin, ymin, xmax, ymax = get_bbox_from_corners(abs_corners)
        xmin = max(0, min(xmin, canvas.width))
        ymin = max(0, min(ymin, canvas.height))
        xmax = max(0, min(xmax, canvas.width))
        ymax = max(0, min(ymax, canvas.height))

        # 1. 转换基础 BBox (拿回 class, x, y, w, h)
        bbox = [xmin, ymin, xmax, ymax]
        label = to_yolo_format(bbox, canvas.width, canvas.height)

        # 2. 计算这 4 个角点的归一化坐标
        norm_corners = [(x / canvas.width, y / canvas.height) for x, y in abs_corners]

        # 3. 拼接 17 值字符串 (class x y w h  x1 y1 2 x2 y2 2 x3 y3 2 x4 y4 2)
        line = f"{label[0]} {label[1]:.6f} {label[2]:.6f} {label[3]:.6f} {label[4]:.6f}"
        for nx, ny in norm_corners:
            line += f" {nx:.6f} {ny:.6f} 2"

        yolo_lines.append(line)  # 存入列表
    ...
    # Save Label (.txt) - 直接把存好的整行字符串写进去，别忘了加换行符 \n
    # --- 修正后的保存逻辑 ---
    with open(os.path.join(DATASET_ROOT, "labels", split, f"{file_name}.txt"), "w") as f:
        for line in yolo_lines:
            ...
            if len(parts) == 17:
                ...
                print(f"警告：检测到非法行，已跳过: {line}")

    print(f"[{split}] 已生成 {file_name}.jpg (含 17 值角点标注)")

# ====================== 4. 生成 data.yaml (本地运行，配置 Colab 路径) ======================
import yaml


def generate_yaml(dataset_root, colab_path="/content/yolov8_dataset"):
    """
    在本地生成 data.yaml 文件
    :param dataset_root: 本地数据集的根目录
    :param colab_path: 未来在 Colab 上的解压路径
    """
    yaml_path = os.path.join(dataset_root, "data.yaml")

    config = {
        # ❗ 这里的 path 是你未来在 Colab 解压后的绝对路径
        'path': colab_path,
        'train': 'images/train',
        'val': 'images/val',
        ...
        # Pose 关键点专属配置
        # 4 代表你有 4 个角点，3 代表每个点包含 (x, y, visible)
        'kpt_shape': [4, 3],

        # 左右翻转索引 (收据不需要左右对称翻转，留空)
        'flip_idx': []
    }

    with open(yaml_path, 'w', encoding='utf-8') as f:
        # 使用 safe_dump 生成干净的 YAML 格式
        yaml.safe_dump(config, f, default_flow_style=False, sort_keys=False)

    print(f"\n✅ 已在本地成功生成 data.yaml")
    print(f"📍 本地路径: {yaml_path}")
    print(f"🚀 云端路径指向: {colab_path}")
