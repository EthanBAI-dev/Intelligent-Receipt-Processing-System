...
logging.basicConfig(
    level=logging.INFO,
    format='[OCR-LOG] %(asctime)s [%(levelname)s] %(message)s',
    datefmt='%Y-%m-%d %H:%M:%S',
    stream=sys.stderr,
)
...
def perform_ocr_on_folder(label_dir, image_dir, output_csv_base):
    logger.info(f"Starting OCR: label_dir={label_dir}, image_dir={image_dir}, output_csv_base={output_csv_base}")
    ocr = get_ocr()
    results = []
    ...
    for label_file in label_files:
        ...
        for line in lines:
            ...
            try:
                ...
                logger.error(f"OCR failed for {label_file} {cname}: {e}")
                extracted_text = ""
    ...
    if not results:
        logger.warning("No OCR results to save.")
        return None
    ...
    logger.info(f"OCR results saved to {output_csv} ({len(results)} entries)")
    return output_csv
...
if __name__ == "__main__":
    ...
    logger.info(f"Using labels from: {label_dir}")
    logger.info(f"Image directory: {args.image_dir}")
    logger.info(f"Output CSV base name: {args.output_csv}")

    result_path = perform_ocr_on_folder(label_dir, args.image_dir, args.output_csv)
    if result_path:
        logger.info(f"OCR completed successfully. Results: {result_path}")
    else:
        logger.warning("OCR completed with no results.")
