...
MODEL_PATH = os.path.join(BASE_DIR, 'logs', 'Epoch82-Total_Loss0.0004.pth')  # Modify to your trained model path
...
prediction_folder = os.path.join(BASE_DIR, 'data', 'prediction')

# Check if model file exists before loading
if not os.path.isfile(MODEL_PATH):
    raise FileNotFoundError(
        f"Model file not found at: {MODEL_PATH}\n"
        f"Please ensure the classification model weights file 'Epoch82-Total_Loss0.0004.pth' "
        f"exists in the 'logs/' directory of the project."
    )
...
# Get all image files from the folder (if it exists)
image_files = []
...
def predict_and_correct_images(images, model, transform, save_folder=None):
    """
    ...
    # Default save folder relative to this file
    if save_folder is None:
        save_folder = os.path.join(BASE_DIR, "data", "corrected_images")
